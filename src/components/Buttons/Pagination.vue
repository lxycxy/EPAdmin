...
const props = defineProps(['pageCount', 'total', 'currentPage'])
const emits = defineEmits(['pageChange'])

const changeCurrentPage = (index : number) => {
  if (index < 1 || index > props.pageCount) return
...
<template>
  <div class="flex items-center justify-between border-t border-slate-200 dark:bg-form-strokedark bg-white px-4 py-3 sm:px-6">
    <div class="flex flex-1 justify-between sm:hidden">
      <a @click="changeCurrentPage(props.currentPage -1)" href="#" class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">上一页</a>
      <a @click="changeCurrentPage(props.currentPage + 1)" href="#" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">下一页</a>
    </div>
    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
      ...
      <div>
        <nav class="isolate inline-flex  -space-x-px rounded-md shadow-sm" aria-label="Pagination">
          <a @click="changeCurrentPage(props.currentPage -1)" class="relative hover:bg-slate-200 hover:scale-110 transform duration-200 inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-slate-200 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
          ...
          <a
              v-if="pageCount > 1"
              @click="changeCurrentPage(props.pageCount)"
              :class="currentPage == props.pageCount ? 'bg-primary text-white' : ''"
              class="relative hover:bg-slate-200 hover:scale-110 transform duration-200 inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-slate-200 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
            <span class="w-5 h-5 flex items-center justify-center">{{props.pageCount}}</span>
          </a>

          <a @click="changeCurrentPage(props.currentPage + 1)" class="relative hover:bg-slate-200 hover:scale-110 transform duration-100 inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-slate-200 hover:bg-gray-50 focus:z-20 focus:outline-offset-0">
